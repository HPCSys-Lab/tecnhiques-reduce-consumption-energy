...
Memory=50G
declare -a algs=("meta.OzaBagExecutorMAXChunk" "meta.OzaBagAdwinExecutorMAXChunk" "meta.AdaptiveRandomForestExecutorMAXChunk")
    ...
    do
      ...
      do
        ...
        do
          ...
          echo "$RESULT_DIR/timedchunk-${i2##*\/}-${j##*.}-${k}-${l}-${m}-${3}"
          echo "$RESULT_DIR/timedchunk-${i2##*\/}-${j##*.}-${k}-${l}-${m}-${3}" >> exper_order.log
          echo ""
          ssh gcassales@192.168.0.11 java ServerProducer 192.168.0.11 9004 ${REMOTE_DIR}${i##*\/} $3 0 &
          sleep 1
          numactl --physcpubind=${cpus[l]} java -Xshare:off -XX:+UseParallelGC -Xmx$Memory -cp $MOA_HOME/lib/:$MOA_HOME/lib/moa.jar moa.DoTask "SocketExperChunksTIMED -l ($j -s $k -c $l) -s (ArffFileStream -f $i) -c $m -e (BasicClassificationPerformanceEvaluator -o -p -r -f) -i -1 -d $RESULT_DIR/dump-timedchunk-${onlyname}-${j##*.}-${k}-${l}-${m}-${3}" > ${RESULT_DIR}/term-timedchunk-${onlyname}-${j##*.}-${k}-${l}-${m}-${3}
          echo ""
        done
      done
    done
