#!/bin/bash
#the idea of this script is to run the experiments until completion to get pradictive performance and task execution time (while getrates runs for a limited time)
...
function Y {
  ...
  if [[ ${alg} == *"MAX"* ]]; then
    ...
    numactl --physcpubind=${CORES} java -Xshare:off -XX:+UseParallelGC -Xmx$Memory -cp $MOA_HOME/lib/:$MOA_HOME/lib/moa.jar moa.DoTask "EITTTExperiments -l ($2 -s ${esize} -c ${nCores}) -s (ArffFileStream -f $1) -e (BasicClassificationPerformanceEvaluator -o -p -r -f) -i -1 -d $RESULT_DIR/dump-${dataset}-${alg}-${esize}-${nCores}-1-1" > ${RESULT_DIR}/term-${dataset}-${alg}-${esize}-${nCores}-1-1
  else
    ...
    numactl --physcpubind=0 java -Xshare:off -XX:+UseParallelGC -Xmx$Memory -cp $MOA_HOME/lib/:$MOA_HOME/lib/moa.jar moa.DoTask "EITTTExperiments -l ($2 -s ${esize}) -s (ArffFileStream -f $1) -e (BasicClassificationPerformanceEvaluator -o -p -r -f) -i -1 -d $RESULT_DIR/dump-${dataset}-${alg}-${esize}-1-1-1" > ${RESULT_DIR}/term--${dataset}-${alg}-${esize}-1-1
  fi
  echo ""
}

function X {
  #this function merely converts the acronym passed as argument to the full class name used in the Task
  #and calls the Y function for each algorithm.
...
#the notebook get_rates helps in the sense that it prints the calls and we only have to paste here
