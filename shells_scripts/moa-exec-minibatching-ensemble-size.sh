...
declare -a ensemble_size=($ensem_size 50 75 100)
...
function Y {
    ...
    for ensem_size in "${ensemble_size[@]}"
    do
        if [[ $2 == *"MAX"* ]]; then
            IDENT="chunk"
            echo "$RESULT_DIR/$FREQUENCIA_MINIMA/$FREQUENCIA_MAXIMA/$3/${IDENT}-${onlyname}-${2##*.}-$ensem_size-4-50-1"
            java -Xshare:off -XX:+UseParallelGC -Xmx$Memory -cp $MOA_HOME/lib/:$MOA_HOME/lib/moa.jar moa.DoTask "EvaluateInterleavedTestThenTrainChunks -l ($2 -s $ensem_size -c 3) -s (ArffFileStream -f $1) -t 120 -c 50 -e (BasicClassificationPerformanceEvaluator -o -p -r -f) -i -1 -d $RESULT_DIR/$FREQUENCIA_MINIMA/$FREQUENCIA_MAXIMA/$3/dump-${onlyname}-${2##*.}-$ensem_size-4-50-1" > ${RESULT_DIR}/$FREQUENCIA_MINIMA/$FREQUENCIA_MAXIMA/$3/term-${IDENT}-${onlyname}-${2##*.}-$ensem_size-4-50-1
        elif [[ ${2} == *"RUNPER"* ]]; then
            IDENT="interleaved"
            echo "$RESULT_DIR/$FREQUENCIA_MINIMA/$FREQUENCIA_MAXIMA/$3/${IDENT}-${onlyname}-${2##*.}-$ensem_size-4-1-1"
            java -Xshare:off -XX:+UseParallelGC -Xmx$Memory -cp $MOA_HOME/lib/:$MOA_HOME/lib/moa.jar moa.DoTask "EITTTExperiments -l ($2 -s $ensem_size -c 3) -s (ArffFileStream -f $1) -t 120 -e (BasicClassificationPerformanceEvaluator -o -p -r -f) -i -1 -d $RESULT_DIR/$FREQUENCIA_MINIMA/$FREQUENCIA_MAXIMA/$3/dump-${onlyname}-${2##*.}-$ensem_size-4-1-1" > ${RESULT_DIR}/$FREQUENCIA_MINIMA/$FREQUENCIA_MAXIMA/$3/term-${IDENT}-${onlyname}-${2##*.}-$ensem_size-4-1-1
        else
            IDENT="interleaved"
            echo "$RESULT_DIR/$FREQUENCIA_MINIMA/$FREQUENCIA_MAXIMA/$3/${IDENT}-${onlyname}-${2##*.}-$ensem_size-1-1-1"
            java -Xshare:off -XX:+UseParallelGC -Xmx$Memory -cp $MOA_HOME/lib/:$MOA_HOME/lib/moa.jar moa.DoTask "EITTTExperiments -l ($2 -s $ensem_size) -s (ArffFileStream -f $1) -t 120 -e (BasicClassificationPerformanceEvaluator -o -p -r -f) -i -1 -d $RESULT_DIR/$FREQUENCIA_MINIMA/$FREQUENCIA_MAXIMA/$3/dump-${onlyname}-${2##*.}-$ensem_size-1-1-1" > ${RESULT_DIR}/$FREQUENCIA_MINIMA/$FREQUENCIA_MAXIMA/$3/term-${IDENT}-${onlyname}-${2##*.}-$ensem_size-1-1-1
        fi
    done
