...
function Y {
  ...
  if [[ $2 == *"MAX"* ]]; then
    ...
    java -Xshare:off -XX:+UseParallelGC -Xmx$Memory -cp $MOA_HOME/lib/:$MOA_HOME/lib/moa.jar moa.DoTask "EITTTExperiments -l ($2 -s 25 -c 4) -s (ArffFileStream -f $1) -e (BasicClassificationPerformanceEvaluator -o -p -r -f) -i -1 -d $RESULT_DIR/$3/dump-${onlyname}-${2##*.}-25-4-1-1" > ${RESULT_DIR}/$3/term-${IDENT}-${onlyname}-${2##*.}-25-4-1-1
  else
    IDENT="interleaved"
...
export RESULT_DIR=/home/pi/reginaldojunior/experimentos/results/speedup-all-algorithms/$CPUS
export REMOTE_DIR=/home/pi/reginaldojunior/comparison-xue3m-minibatching

# alterar para o caminho do HD/scratch
mkdir -p /home/pi/reginaldojunior/experimentos/results/speedup-all-algorithms
mkdir -p /home/pi/reginaldojunior/experimentos/results/speedup-all-algorithms/$CPUS/first \
         /home/pi/reginaldojunior/experimentos/results/speedup-all-algorithms/$CPUS/second \
         /home/pi/reginaldojunior/experimentos/results/speedup-all-algorithms/$CPUS/third
