...
do
	...
	cpufreq-set -c 0,1,3 -d 600MHz -u 600MHz -g powersave
	./moa-exec.sh -f 600 -F 600

	echo "Rodando na frequencia 800MHz x 800MHz com a governança powersave"
	cpufreq-set -c 0,1,3 -d 800MHz -u 800MHz -g powersave
	./moa-exec.sh -f 800 -F 800
	
	echo "Rodando na frequencia 1000MHz x 1000MHz com a governança powersave"
	cpufreq-set -c 0,1,3 -d 1000MHz -u 1000MHz -g powersave
	./moa-exec.sh -f 1000 -F 1000

	echo "Rodando na frequencia 1200MHz x 1200MHz com a governança powersave"
	cpufreq-set -c 0,1,3 -d 1200MHz -u 1200MHz -g powersave
	./moa-exec.sh -f 1200 -F 1200
done

echo "Retorna padrão do CPU"
cset shield --reset --force
cpufreq-set -c 0,1,3 -d 600MHz -u 1200MHz